    ...
    >
      ...
      <g v-for="(flavor, index) in availableFlavors" :key="flavor.id">
        <path
          :id="`arc_path${index + 1}`"
          :d="
            drawTextPath(degrePerArc * index, degrePerArc * (index + 1), 0.85)
          "
          opacity="0"
        ></path>
        <text>
...
"
        data-clickable="true"
        :points="
          `${backButton * 1.6},${backButton / 2} ${backButton *
            1.3},${backButton} ${backButton * 1.6},${backButton * 1.5}`
        "
      ></polyline>
  ...
  methods: {
    ...
    checkFlavor(draggableElement) {
      ...
      if (
        ...
        this.$emit('update-flavor', this.availableFlavors.slice(flavorIndex)[0])
      }
      // console.log(
